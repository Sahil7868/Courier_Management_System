...
public class customer {
	...
	public static void cust_display() 
	{
		...
		if(conn!=null)
		{
			try 
			{
				...
				pstmt=conn.prepareStatement("select * from public.customer v,public.state s,public.city c,public.area a,public.address ad,public.country cs where v.address_id=ad.address_id and ad.area_id=a.area_id and a.city_id=c.city_id and c.state_id=s.state_id and s.country_id=cs.country_id");
	...
	public static boolean cust_insert()
	{
		Connection conn=Dbconnection.getDbConnection();
		ResultSet rs=null;
		ResultSet rs1=null;
	  ...
	  String mob=new String();;
		...
		if(conn!=null)
		{
			...
			{	
				...
				pstmt=conn.prepareStatement("select email_id from public.customer");
				rs=pstmt.executeQuery();
				while(rs.next())
				{
				...
				if(flag==0)
				{
					...
					rs=pstmt1.executeQuery();
						...
						pstmt3=conn.prepareStatement("insert into customer(name1,mobile_num,email_id,gender,password1,address_id) values(?,?,?,?,?,?)");
	...
	public static void main(String[] args) {
		...
		System.out.println("1 for Cutomer display");
		System.out.println("2 for Customer Insert");
